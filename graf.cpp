...
std::vector<int> Graph::dfs(int startNode)
{
    ...
    while (!s.empty())
    {                                        // while the stack is not empty means that we still have nodes to visit
...
std::vector<int> Graph::bfs(int startNode)
{
    ...
    while (!q.empty())
    {                                        // while the queue is not empty means that we still have nodes to visit
...
int Graph::dijkstra(std::vector<int> startNodes, std::vector<int> endNodes)
{
    ...
    while (!pq.empty())
    {
        ...
        { // for every edge
            ...
            {                                  // if the first node of the edge is the current node then:
                int nextNode = edges[i].node2; // the next node is the second node of the edge
                int weight = edges[i].weight;
                if (dist[nextNode] > dist[currentNode] + weight)
                {                                                       // if the distance from the next node is bigger than the distance from the current node + the weight of the edge then:
    ...
    for (int i = 0; i < numNodes; i++)
    {
        ...
        { // while the queue is not empty means that we still have nodes to color
            ...
            for (int i = 0; i < adj[currentNode].size(); ++i)
            {
                ...
                {                                               // if the next node is not colored then:
...
std::vector<int> Graph::topologicalSort()
{
    ...
    while (!q.empty())
    {                                                    // while the queue is not empty means that we still have nodes to add to the topological sort
        int currentNode = q.front();                     // the current node is the first node in the queue
        q.pop();                                         // removing the current node from the queue
